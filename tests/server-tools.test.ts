...
test("each consultation resolves to its own role agent", async () => {
  ...
  assert.deepEqual(agents, [
    "cycle:architect",
    "cycle:executor-advisor",
    "cycle:functional-reviewer",
    "cycle:security-reviewer",
    "cycle:arbiter",
  ])
})
...
test("an unconfigured role reports inheritance rather than inventing a model", async () => {
...
test("a configured role reports its model and how to pass it", async () => {
  const [response] = await exchange([call(1, "role_settings", { consultation: "judge" })], {
    options: { ARBITER_EFFORT: "xhigh", ARBITER_MODEL: "gpt-5.6-sol" },
  })
...
test("permissions reports every role, its boundary and the three enforcement layers", async () => {
  ...
  for (const boundary of result.boundaries) {
    assert.ok(boundary.cannot.includes("Task"), `${boundary.role} must be denied Task`)
  }
