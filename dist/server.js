...
import { verify } from "./evidence/engine.js";
...
const doctor = {
    description: "Report the Cycle installation state: runtime, storage, store schema, per-role model " +
...
const roleSettings = {
    description: "Resolve the agent, model and effort configured for one advisory role. Call this before " +
...
const permissions = {
    ...
    run() {
        return {
            ...
            invariant: "Approval and delivery exist only inside a governed cycle with recorded evidence. No role " +
                "approves its own work, and no configuration changes that.",
        };
    },
};
const recordEvent = {
    description: "Append one action to the tamper-evident project history. Records who did what and when. " +
    ...
    run(args) {
        ...
        const entry = appendHistory(database, {
...
const indexTool = {
    description: "Build or refresh the project's code graph. Incremental: a file whose bytes have not changed " +
...
const graphTool = {
    description: "Query the code graph without loading the repository. `symbol` finds a definition by name, " +
    ...
    run(args) {
        ...
        switch (args["operation"]) {
            ...
            case "scope":
                return scopeBundle(database, project, requirePaths(paths), typeof args["budgetBytes"] === "number" ? args["budgetBytes"] : undefined);
            default:
...
function requireName(args) {
    ...
    if (typeof name !== "string" || !name.trim())
        throw new Error("this operation requires name");
    return name;
}
function requirePaths(paths) {
    if (paths.length === 0)
        throw new Error("this operation requires at least one path");
    return [...paths];
}
...
const MEMORY_OPERATIONS = ["search", "explain", "chain", "forget"];
const memoryTool = {
    description: "Project knowledge derived from completed work and linked to the evidence that justifies it. " +
    ...
    run(args) {
        ...
        switch (args["operation"]) {
            case "search":
                return {
                    memories: recall(context, String(args["query"] ?? ""), Array.isArray(args["paths"]) ? args["paths"] : [], typeof args["limit"] === "number" ? args["limit"] : undefined),
                };
            case "explain":
                return {
                    memories: explain(context, Array.isArray(args["ids"]) ? args["ids"] : []),
                };
            case "chain":
...
const goalTool = {
    description: "A persistent objective above individual workflows. The objective is immutable once created; " +
    ...
    run(args) {
        ...
        const strings = (key) => Array.isArray(args[key]) ? args[key] : [];
        const identifier = () => {
            const value = args["goalId"];
            if (typeof value !== "string" || !value)
                throw new Error("this operation requires goalId");
            return value;
        };
        switch (args["operation"]) {
            case "new":
            ...
            case "link":
                return link(context, identifier(), String(args["milestone"] ?? ""), typeof args["workflowId"] === "string" && args["workflowId"] ? args["workflowId"] : null);
            case "amend":
            ...
            case "extend":
                return extend(context, identifier(), typeof args["additional"] === "number" ? args["additional"] : 1);
            case "pause":
...
const limitsTool = {
    ...
    async run(args) {
        ...
        const identifier = () => {
            ...
            if (typeof value !== "string" || !value)
                throw new Error("this operation requires workflowId");
            return value;
        };
        switch (args["operation"]) {
            case "status":
            ...
            case "admit": {
                ...
                const owner = database.get("select project_id from workflows where id = ?", workflowId);
                if (owner === undefined)
                    throw new Error(`unknown workflow: ${workflowId}`);
                if (owner.project_id !== cycle.project.id) {
                    throw new Error("that workflow belongs to another project");
                }
                return cycle.admission.request(database, cycle.project.id, workflowId, await cycle.resources());
            }
            case "renew":
...
const workflowTool = {
    ...
    async run(args) {
        ...
        switch (args["operation"]) {
            case "start":
                return withFlag(flag, startWorkflow(context, String(args["request"] ?? ""), Array.isArray(args["affectedPaths"]) ? args["affectedPaths"] : [], args["preference"] ?? "auto"));
            case "status":
                return withFlag(flag, workflowStatus(context, typeof args["workflowId"] === "string" ? args["workflowId"] : undefined));
            case "evidence":
                return candidateEvidence(context, id());
            case "submit_plan":
                return submitPlan(context, id(), args["plan"]);
            case "declare_scope":
                return declareScope(context, id(), Array.isArray(args["paths"]) ? args["paths"] : []);
            case "report_task": {
            ...
            case "reconcile":
                return await reconcile(context, cycle.project.path, typeof args["workflowId"] === "string" ? args["workflowId"] : undefined);
            case "recall":
                return recallForRequest(context, String(args["request"] ?? ""), Array.isArray(args["affectedPaths"]) ? args["affectedPaths"] : []);
            case "history":
                return historyState(context, typeof args["limit"] === "number" ? args["limit"] : 50, typeof args["after"] === "number" ? args["after"] : undefined);
            case "control": {
                const operation = args["controlOperation"] ?? "pause";
                if (operation === "cancel" && args["confirm"] !== true) {
                    throw new Error("cancelling a workflow requires confirm: true");
                }
                return control(context, id(), operation, typeof args["reason"] === "string" ? args["reason"] : undefined);
            }
            case "export": {
                if (args["confirm"] !== true) {
                    throw new Error("exporting requires confirm: true");
                }
                return exportState(context, args["kind"] ?? "state", typeof args["workflowId"] === "string" ? args["workflowId"] : undefined);
            }
            default:
