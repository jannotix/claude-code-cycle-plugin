...
async function probeModels(configuration, environment, findings) {
    ...
    if (subagentModelOverride !== null) {
        findings.push({
            ...
            message: `CLAUDE_CODE_SUBAGENT_MODEL is set to "${subagentModelOverride}". It overrides every role ` +
    ...
    if (paths.credentialVariable !== null) {
        findings.push({
            ...
            message: `${paths.credentialVariable} is set, so it replaces the saved login for this session and ` +
    ...
    if (paths.unroutable.length !== 0) {
        findings.push({
            ...
            message: `${paths.unroutable.join(", ")} names a provider, but ANTHROPIC_BASE_URL is not set to a ` +
    ...
    if (availableModelsAllowlist !== null) {
        ...
        if (blocked.length !== 0) {
            findings.push({
                ...
                message: `availableModels does not permit ${blocked.join(", ")}. Claude Code substitutes a ` +
